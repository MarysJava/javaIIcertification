...
class BlockerQueue {
	...
	// await insertion of an element. Once an element is inserted, notify to any
	// threads waiting for insertion in a full queue
	public Object remove() {
		...
		try {
			if (currPointer == 0) {
				...
				// cannot remove - no elements in the queue so block until an element is inserted
				empty.await(); //Lock/Unlock the lock but await/signal the CONDITION! IMPORTANT!!!
				//AWAIY IN THE CURRENT CONDITION
			...
			full.signalAll(); //SIGNAL IN THE CONDITIONS THAT ARE WAITING
	...
	public void insert(Object element) {
		...
		try {
			if (currPointer == size) {
				...
				// cannot insert - the queue is full so block until an element is removed
				full.await();
				// if control reaches here, that means some thread completed
